...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        std::cin >> a[i];
        if (i > 0 && (a[i].y < a[0].y || (a[i].y == a[0].y && a[i].x < a[0].x))) {
            std::swap(a[0], a[i]);
        }
    }
    std::sort(a.begin() + 1, a.end(), [&](const auto &x, const auto &y) {
        int p = cross(x - a[0], y - a[0]);
        if (p > 0 || (p == 0 && distance(a[0], x) < distance(a[0], y))) {
            return 1;
        } else {
            return 0;
        }
    ...
    for (int i = 1; i < a.size(); i++) {
        int m = d.size();
            while (m > 1 && cross(d[m - 1] - d[m - 2], a[i] - d[m - 1]) <= 0) {
            d.pop_back();
            m--;
        }
        d.emplace_back(a[i]);
    ...
    for (int i = 0, j = 2; i < m; i++) {
        while (j < i + m && cross(d[(i + 1) % m] - d[i], d[j % m] - d[i]) <= cross(d[(i + 1) % m] - d[i], d[(j + 1) % m] - d[i])) {
            j++;
        }
        ans = std::max({ans, square(d[i] - d[j % m]), square(d[(i + 1) % m] - d[j % m])});
    }
    std::cout << ans << "\n";
}   
